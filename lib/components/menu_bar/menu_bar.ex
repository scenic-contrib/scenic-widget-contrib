defmodule ScenicWidgets.MenuBar do
  ...
  def validate(%{
          frame: %ScenicWidgets.Core.Structs.Frame{} = _f, # The %Frame{} struct describing the rectangular size of the component
          menu_opts: _map,            # A list containing the contents of the Menu, and what functions to call if that item gets clicked on
          item_width: {:fixed, _w},   # `{:fixed, x}` which means each menu item is a fixed width
          font: %{
            name: _name,              # The font name
            metrics: _fm,             # A %FontMetrics{} struct for this font
            size: _s},                # The font-size of the main menubar options
          sub_menu: %{
            height: _h,               # The height of the sub-menus (as opposed to the main menu bar)
            font: %{size: _font_s}    # The size of the sub-menu font
          }
  } = data) do
  ...
  def render(%{size: {width, height}}, %{mode: :inactive} = args, theme) do
      menu_items_list = args.menu_opts
  ...
  def render_sub_menu(graph, %{state: state, index: top_index, frame: frame, theme: theme}) do
      ...
      |> Scenic.Primitives.group(fn graph ->
          ...
          |> Scenic.Primitives.rect({sub_menu_width, frame.dimensions.height+sub_menu_height}, translate: {0, -frame.dimensions.height}, stroke: {2, theme.border})
           ...
           id: :sub_menu, translate: {menu_item_width*(top_index-1), frame.dimensions.height}
        ])
  end
  ...
  def handle_cast({:click, {:top_index, top_ii, :sub_index, sub_ii}}, %{assigns: %{state: %{menu_opts: menu_opts}}} = scene) do
     {_label, sub_menu} = menu_opts |> Enum.at(top_ii-1) #REMINDER: I use indexes which start at 1, Elixir does not :P 
